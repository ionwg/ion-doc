...
hypermedia client so it can properly collect and then submit the data.

An Ion Form reflects name/value data pairs that may be submitted to a linked resource location. Each name/value pair,
along with any additional information (such as constraints) for that pair, is collectively called a form _field_.  An
...
TBD.  Need to cover "type":"form" and "method" meta members.
