...
Ion codifies specific name/value pairs within JSON objects to better facilitate JSON-based data and metadata exchange
between machines.  And while machine interchange is the primary goal, Ion's second most important design goal is to
enable intuitive and context-relevant structure so Ion content is very easy to understand for humans, even if they do
not know what Ion is.
