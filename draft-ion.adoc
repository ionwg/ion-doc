...
context is required for machine clients and servers to automate information discovery
and exchange via hypermedia controls.

Ion codifies specific name/value pairs within JSON objects to better facilitate JSON-based hypermedia
exchange between machines.  And while machine data and metadata interchange is the primary goal, Ion's second most
important design goal is to enable intuitive and context-relevant structure so Ion content is very easy to understand
for humans, even if they do not know what Ion is.
...
The field `value` must be a JSON string that conforms to the ISO 8601 profile defined in https://www.ietf.org/rfc/rfc3339.txt[RFC 3339].
...
The field `value` must be a JSON string and user agents should mask the characters so they are not directly visible to a user.
