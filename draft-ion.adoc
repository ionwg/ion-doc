...
.Resource
The referent of a {uri-IRIs}[International Resource Identifier].
...
----
{
  ...
  "items": [
    ...
    { "name": "visitedContinents", "type": "set", "itemtype": "string", "minitems": 1, "options": {
        "items": [
...
An Ion Form MAY NOT be an Ion Link if and only if that form is nested inside another form or form field.  Nesting forms
...
1. The JSON object has an `items` array member with a value that is not `null` or empty.
1. The JSON object's `items` array contains one or more Ion Form Field objects.
1. The JSON object's `items` array does not contain elements that are not Ion Form Field objects.
...
The `maxitems` member value is non-negative integer that specifies the maximum number of field values that may be
...
The `minitems` member value is non-negative integer that specifies the minimum number of field values that may be
...
A non-null field `value` must be a JSON string that is a `base64`-encoded octet sequence according to https://tools.ietf.org/html/rfc4648[RFC 4648].  The string MUST NOT contain characters that are not in the `base64` alphabet.
...
A non-null field `value` must be a JSON string and user agents MUST mask the characters so they are not visible to a user.
