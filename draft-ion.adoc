...
The `method` member identifies the {uri-rfc7231-methods}[HTTP method] to be used when sending an HTTP request to the resource identified by the associated `href` member.  The `method` value is a case-sensitive string as defined by {uri-rfc7231-methods}[RFC 7231 Section 4.1] and safety and idempotent semantics are equivalent to those specified in RFC 7231.
...
The `accepts` member identifies one or more supported https://tools.ietf.org/html/rfc6838[Media Type]s of representations that may be submitted to the resource identified by the associated `href` member. The value is a JSON array of strings.  Each string in the array MUST be a case-insensitive string as defined by {uri-mediaType-namingReqs}[RFC 6838 Section 4.2].
...
The `produces` member identifies one or more HTTP response https://tools.ietf.org/html/rfc6838[Media Type]s for representations that may be returned from the resource identified by the associated `href` member.  The value is a JSON array of strings.  Each string in the array MUST be a case-insensitive string as defined by {uri-mediaType-namingReqs}[RFC 6838 Section 4.2].
