...
1. MAY recognize members which are not defined in the Ion Object Member Registry,
1. MUST ignore members which it does not understand.
...
1.  If a form field has an `object` type and a `form` member, that form field's `value` member will first be serialized
...
1.  If the form does not contain an `accepts` member, the Form Submission Object SHOULD be identified as either
    `application/json` or `application/ion+json` content type.
...
According to the example form, the associated Form Submission Object may be transmitted to the linked resource location
of `https://example.io/users` using an HTTP `POST` and a content-type of either `application/json` or `application/ion+json`.
