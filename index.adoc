...
This Internet-Draft will expire on 31 December 2016.

.Copyright Notice

Copyright (c) 2013-2016 Ion Working Group and persons identified as document authors.  All rights reserved.
...
.Form
An _Collection Object_ that contains Form Fields.
...
Most JSON objects are parsed as as expected:
...
relevant and support the value.  For example, one might add a `lang` member to indicate a spoken language:
...
An Ion _Link_ is a JSON serialization of a {uri-rfc5988}[web link] to another resource.
...
the `value` array itself or the elements in the `value` array, something not possible with standard JSON grammar.
...
.Example Collection Object With Metadata:
[source,json]
----
{
  "itemtype": "person"
...
In addition to this metadata, Collection Objects can have other members to build up rich functionality.  For example,
...
Other members can be added to build up rich functionality.  For example, a Collection Resource with
...
An Ion Form is a _Collection Object_ that contains _Form Fields_.  Ion Forms ensure that resource transitions (links)
that support data submissions can be discovered automatically (colloquially referred to as
...
The member names within an identified _Meta Object_ SHOULD be unique; Ion parsers MUST either reject _Meta Object_
...
contextual information regarding the `value` member.
...
`href` location.  The `form` value's `meta` `href` location may only be used to read the associated form to
